...
impl ToCbml for Stmt {
    fn to_cbml(&self, deepth: usize) -> String {
        self.kind.to_cbml(deepth)
    }
}

impl ToCbml for Vec<StmtKind> {
    fn to_cbml(&self, deepth: usize) -> String {
        let mut re = String::new();
...
impl ToCbml for StmtKind {
    fn to_cbml(&self, deepth: usize) -> String {
        match self {
            StmtKind::Use(use_stmt) => use_stmt.to_cbml(deepth),
            StmtKind::Asignment(asignment_stmt) => asignment_stmt.to_cbml(deepth),
            StmtKind::FileFieldStmt(struct_field_def_stmt) => struct_field_def_stmt.to_cbml(deepth),
            StmtKind::TypeAliasStmt(type_alias_stmt) => type_alias_stmt.to_cbml(deepth),
            StmtKind::StructDefStmt(struct_def) => struct_def.to_cbml(deepth),
            StmtKind::EnumDef(enum_def) => enum_def.to_cbml(deepth),
            ...
            StmtKind::TypeDef(type_def_stmt) => match type_def_stmt {
                ...
                TypeDefStmt::StructDefStmt(struct_def) => struct_def.to_cbml(deepth),
                TypeDefStmt::EnumDef(enum_def) => enum_def.to_cbml(deepth),
                TypeDefStmt::UnionDef(union_def) => union_def.to_cbml(deepth),
            },
        }
    }
}

impl ToCbml for UseStmt {
    fn to_cbml(&self, deepth: usize) -> String {
        _ = deepth;
...
impl ToCbml for TypeAliasStmt {
    fn to_cbml(&self, deepth: usize) -> String {
        let mut re = String::new();
        re.push_str(&format!("type {} = ", self.name));
        re.push_str(&self.ty.to_cbml(deepth));
        re.push_str("\n");

        return re;
    }
}
impl ToCbml for AsignmentStmt {
    fn to_cbml(&self, deepth: usize) -> String {
        let mut re = String::new();
        ...
        re.push_str(&format!(
            ...
            "    ".repeat(deepth),
            self.field_name,
            self.value.kind.to_cbml(deepth)
        ));
...
impl ToCbml for Vec<StructFieldDefStmt> {
    fn to_cbml(&self, deepth: usize) -> String {
        let mut re = String::new();
...
impl ToCbml for StructFieldDefStmt {
    fn to_cbml(&self, deepth: usize) -> String {
        let mut re = String::new();
        ...
        re.push_str(": ");
        re.push_str(&self._type.kind.to_cbml(deepth));
...
impl ToCbml for EnumFieldDef {
    fn to_cbml(&self, deepth: usize) -> String {
        let mut re = String::new();
        re.push_str(&"    ".repeat(deepth));
        re.push_str(&self.field_name);
        re.push_str("(");
        re.push_str(&self._type.to_cbml(deepth));
        re.push_str(") ");
...
impl ToCbml for Literal {
    fn to_cbml(&self, deepth: usize) -> String {
        self.kind.to_cbml(deepth)
    }
}

impl ToCbml for LiteralKind {
    fn to_cbml(&self, deepth: usize) -> String {
        match self {
            ...
            LiteralKind::Array(literals) => {
                ...
                for l in literals {
                    ...
                    re.push_str(&format!("{}{}", l.to_cbml(deepth), a));
                    count += 1;
                }
            ...
            LiteralKind::Struct(asignment_stmts) => {
...

                {
                    let newline_style: String = asignment_stmts
                        .iter()
                        .map(|x| x.to_cbml(deepth + 1)) // 每一个 stmt 转换为代码.
                        .fold("\n".to_string(), |mut a, b| {
                            a.push_str(&b);
            ...
            LiteralKind::EnumFieldLiteral {
                ...
                re.push_str(&_literal.to_cbml(deepth));
...
impl ToCbml for TypeSignStmt {
    fn to_cbml(&self, deepth: usize) -> String {
        let mut re = String::new();
        re.push_str(&self.kind.to_cbml(deepth));
        // re.push_str("\n");
...
impl ToCbml for TypeSignStmtKind {
    fn to_cbml(&self, deepth: usize) -> String {
        match self {
            ...
            TypeSignStmtKind::Anonymous(anonymous_type_def_stmt) => {
                match &anonymous_type_def_stmt.kind {
                    ...
                    AnonymousTypeDefKind::Struct(struct_field_def_stmts) => {
                        ...
                        re.push_str(&struct_field_def_stmts.to_cbml(deepth + 1));
                    ...
                    AnonymousTypeDefKind::Union {
                        ...
                        alowd_values.iter().for_each(|x| {
                            ...
                            if counter < alowd_values.len() {
                                str.push_str(&format!("{} | ", x.to_cbml(deepth)));
                            } else {
                                str.push_str(&format!("{} ", x.to_cbml(deepth)));
                            }
                        });
                    ...
                    AnonymousTypeDefKind::Array { inner_type } => {
                        format!("[{}]", inner_type.to_cbml(deepth + 1))
                    }
...
impl ToCbml for StructDef {
    fn to_cbml(&self, deepth: usize) -> String {
        let mut re = String::new();

        re.push_str(&format!("struct {} {{\n", self.struct_name));

        re.push_str(&self.fields.to_cbml(deepth + 1));
...
impl ToCbml for UnionDef {
    fn to_cbml(&self, deepth: usize) -> String {
        let mut re = String::new();
        re.push_str(&format!("union {} = ", self.union_name));

        re.push_str("(");
        re.push_str(&self.base_type.to_cbml(deepth));
        re.push_str(")");
        re.push_str(" = ");

        for x in &self.allowed_values {
            re.push_str(&x.kind.to_cbml(deepth));
            re.push_str(" | ");
        }
